...
const obsPhase1: ObsPhaseData = {
  ...
  edges: [
    ...
    { id: "e-worker-cw", source: "worker", target: "cloudwatch" },
    { id: "e-worker-langfuse", source: "worker", target: "langfuse" },
...
const obsPhase1Steps: FlowStep[] = [
  ...
  {
    id: 2, actor: "Worker", action: "emit STT logs + metrics", target: "CloudWatch",
    detail: "Speech-to-text task logs, error counts, and Fargate resource metrics",
    actorType: "worker", targetType: "monitoring",
    actorZh: "Worker", actionZh: "發送 STT 日誌 + 指標", targetZh: "CloudWatch",
    detailZh: "語音轉文字任務日誌、錯誤計數及 Fargate 資源指標",
  },
  {
    id: 3, actor: "Worker", action: "emit LLM logs + metrics", target: "CloudWatch",
    detail: "Summarization logs, token counts, and the same Fargate task resource metrics",
    actorType: "worker", targetType: "monitoring",
    actorZh: "Worker", actionZh: "發送 LLM 日誌 + 指標", targetZh: "CloudWatch",
    detailZh: "摘要推論日誌、token 計數，以及同一個 Fargate Task 的資源指標",
  },
  {
    id: 4, actor: "Worker", action: "send LLM traces", target: "Langfuse",
    detail: "The same worker records prompt, model, latency, token usage, and response metadata for each managed LLM call",
    actorType: "worker", targetType: "monitoring",
    actorZh: "Worker", actionZh: "傳送 LLM 追蹤", targetZh: "Langfuse",
    detailZh: "同一個 Worker 記錄每次託管 LLM 呼叫的 prompt、模型、延遲、token 使用量與回應中繼資料",
  },
  {
